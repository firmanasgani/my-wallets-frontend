    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <button @click="selectPersonal" :class="cardClass(wsStore.mode === 'personal')">
        <div class="flex items-start justify-between gap-3">
          ...
          <span v-if="wsStore.mode === 'personal'" class="text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/40 px-2 py-0.5 rounded-full">Aktif</span>
        </div>
      ...
      <button @click="selectBusiness" :class="cardClass(wsStore.mode === 'business' && isBusinessPlan)">
        <div class="flex items-start justify-between gap-3">
          ...
          <span v-if="wsStore.mode === 'business' && isBusinessPlan" class="text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/40 px-2 py-0.5 rounded-full">Aktif</span>
        ...
        <div class="mt-3">
          <span v-if="!isBusinessPlan" class="text-xs font-semibold text-indigo-600 dark:text-indigo-400">
            Upgrade ke Business →
          </span>
          <span v-else class="text-xs px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300">Business</span>
        </div>
      </button>
    </div>
